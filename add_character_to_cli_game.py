...
def quick_add_character_examples():
    ...
    agent1 = integrator.add_character_to_game(char1, "resistance", "university_district")
    
    print(f"✅ {char1.name} ({char1.background.name})")
    print(f"   Primary Skills: Hacking({char1.skills.hacking}) Technical({char1.skills.technical})")
    print(f"   Specialization: Technical operations and cyber warfare")
    ...
    agent2 = integrator.add_character_to_game(char2, "urban_liberation", "government_quarter")
    
    print(f"✅ {char2.name} ({char2.background.name})")
    print(f"   Primary Skills: Social({char2.skills.social}) Stealth({char2.skills.stealth})")
    print(f"   Specialization: Infiltration and social manipulation")
    ...
    print(f"   Primary Skills: Combat({char3.skills.combat}) Survival({char3.skills.survival})")
    print(f"   Specialization: Direct action and military operations")  
    print(f"   Game Status: Added to {agent3.faction_id} faction")
    
    print(f"\n📊 UPDATED GAME STATE:")
    print(f"Total Agents: {len(integrator.game_state.agents)}")
    print(f"Resistance: {len([a for a in integrator.game_state.agents.values() if a.faction_id == 'resistance'])} agents")
    print(f"Urban Liberation: {len([a for a in integrator.game_state.agents.values() if a.faction_id == 'urban_liberation'])} agents")  
    print(f"Underground: {len([a for a in integrator.game_state.agents.values() if a.faction_id == 'underground'])} agents")
    ...
    print(f"   1. Start the game: python main.py --mode cli")
    print(f"   2. Use command [2] to see all agents including your custom ones")
    print(f"   3. Use command [4] to assign tasks to your custom characters")
    print(f"   4. Use command [8] to create missions with your custom characters")
...
def create_character_for_specific_mission():
    ...
    try:
        ...
        character = integrator.character_creator.quick_create_character(f"Agent_{mission_type.title()}")
