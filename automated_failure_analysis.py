...
class FailureAnalyzer:
    ...
    def initialize_game_systems(self):
        ...
        try:
            ...
            print(
                f"✅ Systems initialized: {len(self.game_state.agents)} agents, "
    ...
    def test_system_stability(self):
        ...
        # Analyze stability
        avg_time = sum(m["processing_time"] for m in stability_metrics) / len(
            stability_metrics
        )
        ...
        print(
            f"  ⏰ Stability: {avg_time:.3f}s avg turn time, "
    ...
    def analyze_cascade_impact(self, initial_state, final_state):
        ...
        return {
            ...
            if (initial_state["active_agents"] - final_state["active_agents"]) > 0
            else "stable",
        }
    ...
    def analyze_patterns(self):
        ...
        failures = [
            m
            for m in self.data["missions"]
            if "FAILURE" in str(m.get("outcome", "")).upper()
        ]
        ...
        for mission in self.data["missions"]:
            ...
            if mission_type not in mission_analysis["mission_types"]:
                mission_analysis["mission_types"][mission_type] = {
                    "total": 0,
                    "failures": 0,
                }
            ...
            if scenario not in mission_analysis["scenario_performance"]:
                mission_analysis["scenario_performance"][scenario] = {
                    "total": 0,
                    "failures": 0,
                }
        ...
        emotional_analysis = {
            ...
            "high_trauma_agents": len(
                [
                    e
                    for e in self.data["emotional_impact"]
                    if e.get("trauma_level", 0) > 0.5
                ]
            ),
            "unstable_agents": len(
                [
        ...
        for emotional_data in self.data["emotional_impact"]:
            ...
            emotional_analysis["dominant_emotions"][emotion] = (
                emotional_analysis["dominant_emotions"].get(emotion, 0) + 1
            )
        ...
        system_analysis = {
            ...
            "stability_score": self.data.get("system_stability", {}).get(
                "stability_score", 0.0
            ),
            "average_turn_time": self.data.get("system_stability", {}).get(
        ...
        recommendations = self.generate_recommendations(
            mission_analysis, emotional_analysis, system_analysis
        )
        ...
        self.data["analysis"] = {
            ...
            "test_summary": {
                ...
                - (self.error_count / max(self.test_count, 1)),
            },
        }
    ...
    def generate_recommendations(
        self, mission_analysis, emotional_analysis, system_analysis
    ):
        ...
        if mission_analysis["failure_rate"] < 0.2:
            recommendations.append(
                "Mission failure rate is low - consider increasing difficulty or risk factors"
            )
        elif mission_analysis["failure_rate"] > 0.7:
            recommendations.append(
                "Mission failure rate is very high - consider balancing difficulty"
            )

        # Emotional health recommendations
        if (
            emotional_analysis["high_trauma_agents"]
            > emotional_analysis["agents_analyzed"] * 0.5
        ):
            recommendations.append(
                "High trauma levels detected - implement trauma recovery mechanisms"
            )

        if (
            emotional_analysis["unstable_agents"]
            > emotional_analysis["agents_analyzed"] * 0.3
        ):
            recommendations.append(
                "Many unstable agents - review emotional state management"
            )
        ...
        if system_analysis["stability_score"] < 0.8:
            recommendations.append(
                "System stability is concerning - investigate performance bottlenecks"
            )

        if system_analysis["average_turn_time"] > 0.5:
            recommendations.append(
                "Turn processing is slow - optimize game loop performance"
            )
        ...
        for mission_type, stats in mission_analysis["mission_types"].items():
            if stats["total"] > 0:
                ...
                if failure_rate > 0.8:
                    recommendations.append(
                        f"{mission_type} missions have very high failure rate - review balance"
                    )
    ...
    def save_results(self):
        ...
        self.data["metadata"] = {
            ...
            "test_duration": (
                ...
                - datetime.fromisoformat(
                    self.data.get("test_start", datetime.now().isoformat())
                )
            ).total_seconds()
            if "test_start" in self.data
            else 0,
            "game_systems_tested": [
                "missions",
                "emotions",
        ...
        try:
            ...
            with open(summary_filename, "w") as f:
                ...
                f.write(
                    f"  Total Missions: {mission_analysis.get('total_missions', 0)}\n"
                )
                f.write(
                    f"  Failure Rate: {mission_analysis.get('failure_rate', 0):.1%}\n\n"
                )

                f.write("EMOTIONAL IMPACT:\n")
                emotional_analysis = analysis.get("emotional_analysis", {})
                f.write(
                    f"  Agents Analyzed: {emotional_analysis.get('agents_analyzed', 0)}\n"
                )
                f.write(
                    f"  High Trauma: {emotional_analysis.get('high_trauma_agents', 0)}\n"
                )
                f.write(
                    f"  Unstable: {emotional_analysis.get('unstable_agents', 0)}\n\n"
                )
                ...
                f.write(
                    f"  Stability Score: {system_analysis.get('stability_score', 0):.3f}\n\n"
                )
