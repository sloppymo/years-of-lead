...
def create_demo_game_state():
    ...
    resistance_agents = [
        aid
        for aid, agent in game_state.agents.items()
...
def demonstrate_agent_autonomy(game_state):
    ...
    try:
        ...
        for decision in results.get("decisions_made", [])[:3]:
            ...
            if agent:
                print(
                    f"    • {agent.name}: {decision.action_type.value} (Risk: {decision.risk_level:.2f})"
                )
...
def demonstrate_intelligence_system(game_state):
    ...
    try:
        ...
        for intel in results.get("new_intelligence", [])[:3]:
            print(
                f"    📡 {intel.content if hasattr(intel, 'content') else 'Intelligence event'}"
            )
...
def demonstrate_trauma_system(game_state):
    ...
    try:
        ...
        print(
            f"  🧠 Psychological Crises: {len(results.get('psychological_crises', []))}"
        )
        print(
            f"  ✨ Healing Breakthroughs: {len(results.get('healing_breakthroughs', []))}"
        )
        ...
        for trauma in results.get("new_trauma_events", [])[:2]:
            ...
            if agent:
                print(
                    f"    💔 {agent.name}: {trauma.get('description', 'Trauma event')}"
                )
        ...
        for crisis in results.get("psychological_crises", [])[:2]:
            ...
            if agent:
                print(
                    f"    🧠 {agent.name}: {crisis.get('crisis_type', 'Crisis')} - {crisis.get('description', '')}"
                )
...
def demonstrate_mission_system(game_state):
    ...
    try:
        ...
        resistance_agents = [
            a for a in game_state.agents.values() if a.faction_id == "resistance"
        ]
        ...
        result = mission_system.execute_enhanced_mission(
            mission, agents_data, location, resources
        )
        ...
        if collaboration:
            ...
            print(
                f"  🛠️ Skill Complementarity: {collaboration.skill_complementarity:.2f}"
            )
        ...
        print(
            f"  🔗 Relationship Changes: {len(relationship_changes)} relationships modified"
        )
...
def demonstrate_integration_system(game_state):
    ...
    try:
        ...
        for turn_num in range(3):
            ...
            print(
                f"    📊 Autonomy: {len(turn.autonomy_results.get('decisions_made', []))} decisions"
            )
            print(
                f"    📡 Intelligence: {len(turn.intelligence_results.get('new_intelligence', []))} events"
            )
            print(
                f"    📖 Narrative: {len(turn.narrative_results.get('story_events', []))} events"
            )
            print(
                f"    💔 Trauma: {len(turn.trauma_results.get('new_trauma_events', []))} events"
            )
        ...
        for system, status in system_status.items():
            ...
            print(
                f"    {status_icon} {system.title()}: {'Active' if status else 'Inactive'}"
            )
...
def main():
    ...
    print(
        "This demonstration showcases all enhanced simulation systems working together."
    )
    print(
        "Each phase builds upon the previous ones to create a comprehensive simulation experience."
    )
    ...
    print(
        "  🎯 Agent Autonomy: Agents make independent decisions based on emotional state"
    )
