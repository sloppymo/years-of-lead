...
# Import game systems
from game.equipment_enhanced import EnhancedEquipmentManager
from game.equipment_integration import (
    EquipmentIntegrationManager,
    AgentLoadout,
    LoadoutSlot,
)
from game.mission_execution_engine import MissionExecutionEngine
from game.equipment_system import EquipmentCategory
...
class EquipmentIntegrationDemo:
    ...
    def demo_loadout_creation(self):
        ...
        for agent in self.agents:
            ...
            for slot, equipment in loadout.equipment.items():
                print(
                    f"      {slot.value}: {equipment.name} (Condition: {equipment.durability.condition:.2f})"
                )
    ...
    def _equip_agent_based_on_skills(
        self, loadout: AgentLoadout, agent: Dict[str, Any]
    ):
        ...
        for skill, level in best_skills:
            if skill == "stealth":
                ...
                stealth_equipment = [
                    ...
                    if e.category == EquipmentCategory.TOOL
                    and "stealth" in e.effects.skill_bonuses
                ]
                ...
                combat_equipment = [
                    e
                    for e in available_equipment
                    if e.category == EquipmentCategory.WEAPON
                    and "combat" in e.effects.skill_bonuses
                ]
                if combat_equipment:
                    loadout.add_equipment(
                        LoadoutSlot.PRIMARY_WEAPON, combat_equipment[0]
                    )
                ...
                electronic_equipment = [
                    e
                    for e in available_equipment
                    if e.category == EquipmentCategory.ELECTRONIC
                    and "hacking" in e.effects.skill_bonuses
                ]
                if electronic_equipment:
                    loadout.add_equipment(
                        LoadoutSlot.ELECTRONIC, electronic_equipment[0]
                    )
    ...
    def demo_pre_mission_planning(self):
        ...
        for mission in self.missions:
            ...
            print(
                f"      Success Probability Boost: {analysis.success_probability_boost:+.1%}"
            )
    ...
    def demo_mission_execution(self):
        ...
        for mission in self.missions:
            ...
            if degradation:
                print("   🔧 Equipment Degradation:")
                for agent_id, agent_degradation in degradation.items():
                    agent_name = next(
                        a["name"] for a in self.agents if a["id"] == agent_id
                    )
                    print(f"      {agent_name}:")
                    for slot, result in agent_degradation.items():
                        if result["degradation"] < 0:
                            print(
                                f"         {slot}: {result['equipment_name']} - Condition: {result['condition_after']:.2f}"
                            )
                            if result["broken"]:
                                print("            ❌ BROKEN")
                            elif result["maintenance_required"]:
                                print("            ⚠️ Needs Maintenance")
    ...
    def demo_equipment_maintenance(self):
        ...
        print(
            f"   Maintenance Required: {len(maintenance_report['maintenance_required'])}"
        )
        ...
        if maintenance_report["broken_equipment"]:
            ...
            for item in maintenance_report["broken_equipment"]:
                print(
                    f"   {item['equipment']} ({item['agent_id']}) - Repair Cost: ${item['repair_cost']}"
                )
        ...
        if maintenance_report["maintenance_required"]:
            ...
            for item in maintenance_report["maintenance_required"]:
                print(
                    f"   {item['equipment']} ({item['agent_id']}) - Condition: {item['condition']:.2f}"
                )
            ...
            for slot, equipment in loadout.equipment.items():
                if equipment.durability.condition < 1.0:
                    ...
                    if repair_success:
                        ...
                        print(
                            f"         ✅ Repaired! Condition: {original_condition:.2f} → {new_condition:.2f} (+{improvement:.2f})"
                        )
                    else:
                        print("         ❌ Repair failed or not needed")
    ...
    def run_equipment_effectiveness_analysis(self):
        ...
        for mission_type in mission_types:
            ...
            if top_equipment:
                print(
                    f"   Top Equipment: {', '.join([e.name for e in top_equipment[:3]])}"
                )
...
def main():
    ...
    try:
        ...
        print("\n🎊 Demo Complete!")
        print("✅ Phase 1 equipment system successfully integrated")
        print("✅ Equipment affects mission outcomes")
        print("✅ Loadouts provide meaningful bonuses")
        print("✅ Equipment degrades with use")
        print("✅ Maintenance system works")
