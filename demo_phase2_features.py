...
from utils.pattern_engine import PatternEngine
from utils.archetype_engine import ArchetypeEngine
...
def create_sample_interactions():
    ...
    for interaction in sample_interactions:
        ...
        if interaction["user_input"]:
            emotion_counts = memory_logger.pattern_engine.analyze_emotions([interaction["user_input"]])
            detected_emotion = memory_logger.pattern_engine.get_dominant_emotion(emotion_counts)
            ...
            memory_logger.archetype_engine.track_archetype_preference(
                interaction["user_input"], interaction["subsystem"], interaction["archetype"]
            )
...
def demonstrate_phase2_features():
    ...
    ritual_intensity = pattern_engine.determine_ritual_intensity(recent_emotions, subsystem_tension)
    ...
    weighted_interactions = [(i.get("memory_weight", 0.0), i.get("user_input", "")) 
                           for i in interactions[-5:] if i.get("memory_weight")]
