...
class EnhancedAutomatedTesting:
    ...
    def initialize_systems(self) -> bool:
        ...
        try:
            ...
            from game.intelligence_system import IntelligenceDatabase, IntelligenceGenerator
    ...
    def test_basic_functionality(self):
        ...
        self.record_test("basic_game_state", True, {
        ...
        self.record_test("agent_attributes", True, {
            "sample_agent": first_agent.name,
            "status": first_agent.status
        })
        ...
        if agents_with_emotions:
            try:
                ...
                if high_stress_agent:
                    ...
                    print(f"    🔥 Stress test on {high_stress_agent.name}: {result['performance_impact']}")
            except Exception as e:
    ...
    def test_cascading_effects(self):
        ...
        try:
            ...
            print(f"    🌊 Cascade effects detected: {len(cascade_analysis['effects_detected'])}")
    ...
    def test_intelligence_systems(self):
        ...
        try:
            ...
            from game.intelligence_system import IntelligenceType, IntelligencePriority, IntelligenceSource
            ...
            self.record_test("intelligence_systems", True, {
                "events_generated": len(intel_events),
                "patterns_detected": len(patterns)
            })
            
            print(f"    📊 Intelligence: {len(intel_events)} events, {len(patterns)} patterns")
    ...
    def test_long_term_stability(self):
        ...
        # Analyze stability
        avg_turn_time = sum(stability_metrics["turn_processing_times"]) / len(stability_metrics["turn_processing_times"])
        stability_score = 1.0 - (len(stability_metrics["performance_degradation"]) / 20)
        
        self.record_test("long_term_stability", True, {
            "average_turn_time": avg_turn_time,
            "stability_score": stability_score,
            "slow_turns": len(stability_metrics["performance_degradation"])
        })
        
        print(f"    ⏰ Stability: {stability_score:.2f} (avg turn: {avg_turn_time:.3f}s)")
    ...
    def test_stress_scenarios(self):
        ...
        for test_name, test_function in stress_tests:
            try:
                ...
                self.record_test(f"stress_{test_name.lower().replace(' ', '_')}", True, result)
    ...
    def execute_failure_scenario(self, mission_type: str, scenario: Dict) -> Dict:
        ...
        # Execute mission
        agent_data = [{"id": a.id, "name": a.name, "skills": getattr(a, "skills", {})} for a in test_agents]
        location_data = {"name": location.name, "security_level": scenario.get("security_level", 5)}
    ...
    def test_stressed_agent_performance(self, agent) -> Dict:
        ...
        try:
            ...
            agent_data = [{"id": agent.id, "name": agent.name, "skills": getattr(agent, "skills", {})}]
            ...
            return {
                ...
                "success_probability": mission_result.get("success_probability", 0.0),
                "performance_impact": "degraded" if mission_result.get("success_probability", 0.5) < 0.4 else "normal"
            }
        except Exception as e:
            return {"agent_name": agent.name, "error": str(e)}
    ...
    def capture_network_state(self) -> Dict:
        ...
        return {
            ...
            "faction_resources": {f.name: getattr(f, 'resources', 0) for f in self.game_state.factions.values()},
            "timestamp": datetime.now().isoformat()
        }
    ...
    def trigger_cascade_scenario(self, trigger: Dict):
        ...
        affected_count = 0
        for agent in list(self.game_state.agents.values())[:trigger["affected_agents"]]:
            if hasattr(agent, 'emotional_state') and agent.emotional_state:
                # Apply trauma/stress
    ...
    def detect_cascade_effects(self, initial_state: Dict) -> List[str]:
        ...
        if current_state["active_agents"] < initial_state["active_agents"]:
            effects.append(f"Agent deactivation: {initial_state['active_agents'] - current_state['active_agents']} agents")
        
        # Check for resource changes - FIXED TYPE ERROR
        for faction_name, initial_resources in initial_state["faction_resources"].items():
            ...
            if isinstance(initial_resources, dict):
                ...
                current_money = current_resources.get('money', 0) if isinstance(current_resources, dict) else 0
                ...
                if current_resources < initial_resources * 0.9:  # 10% decrease threshold
                    effects.append(f"Resource depletion in {faction_name}")
    ...
    def analyze_cascade_impact(self, initial_state: Dict, final_state: Dict) -> Dict:
        ...
        return {
            ...
            "cascade_severity": "high" if (initial_state["active_agents"] - final_state["active_agents"]) > 1 else "low",
    ...
    def test_relationship_impact(self, agents) -> Dict:
        ...
        return {
            ...
            "summary": "Relationship system detected" if hasattr(self.game_state, 'social_network') else "No relationship system"
        }
    ...
    def stress_test_rapid_missions(self) -> Dict:
        ...
        try:
            ...
            for i in range(5):
                agents_list = list(self.game_state.agents.values())
                if len(agents_list) >= 2:
                    result = self.execute_failure_scenario("intelligence", {"name": "Rapid Test", "security_level": 5})
            ...
            return {
                ...
                "missions_per_second": missions_executed / (end_time - start_time) if end_time > start_time else 0
            }
        except Exception as e:
    ...
    def generate_final_report(self) -> Dict[str, Any]:
        ...
        if len(self.cascading_effects) > 5:
            recommendations.append("Significant cascading effects detected - review failure propagation")
