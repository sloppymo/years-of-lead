...
class CharacterGameIntegrator:
    ...
    def convert_character_to_agent(
        self, character, faction_id="resistance", location_id="university"
    ):
    ...
    def _create_ideology_from_character(self, character):
        ...
        if character.background.type in background_modifiers:
            for key, modifier in background_modifiers[
    ...
    def add_character_to_game(
        self, character, faction_id="resistance", location_id="university_district"
    ):
    ...
    def demonstrate_integration(self):
        ...
        for agent_id, agent in self.game_state.agents.items():
            print(
                f"  • {agent.name} ({agent.background}) - Faction: {agent.faction_id}"
            )
        ...
        print(
            f"Primary Skills: Combat({custom_char.skills.combat}) Social({custom_char.skills.social}) Stealth({custom_char.skills.stealth})"
        )

        # Add character to game
        print(f"\n🔗 Adding {custom_char.name} to the game...")

        agent = self.add_character_to_game(
            custom_char, "resistance", "university_district"
        )
        ...
        for agent_id, agent in self.game_state.agents.items():
            if agent.id == custom_char.id:
                print(
                    f"  🆕 {agent.name} ({agent.background}) - Faction: {agent.faction_id} [NEW!]"
                )
            else:
                print(
                    f"     {agent.name} ({agent.background}) - Faction: {agent.faction_id}"
