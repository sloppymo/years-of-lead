...
class InteractivePlaytestDemo:
    ...
    def setup_playtest_scenario(self):
        ...
        locations = [
            Location(
                "university", "University District", security_level=3, unrest_level=8
            ),
            Location(
                "downtown", "Downtown", security_level=9, unrest_level=2
            ),  # High security
            Location("industrial", "Industrial Zone", security_level=6, unrest_level=7),
            Location(
                "government", "Government Quarter", security_level=10, unrest_level=1
            ),  # Maximum security
        ]
        ...
        factions = [
            Faction(
                "resistance",
                "The Resistance",
                {"money": 100, "influence": 20, "personnel": 5},
            ),
            Faction(
                "students",
                "Student Movement",
                {"money": 30, "influence": 40, "personnel": 8},
            ),
        ]
        ...
        agents = [
            ...
            Agent(
                ...
                background="military",
                skills={
                    SkillType.COMBAT: Skill(SkillType.COMBAT, level=5),
            ...
            Agent(
                ...
                background="student",
                skills={
                    SkillType.PERSUASION: Skill(SkillType.PERSUASION, level=4),
            ...
            Agent(
                ...
                background="civilian",
                skills={
                    SkillType.SURVIVAL: Skill(SkillType.SURVIVAL, level=2),
            ...
            Agent(
                ...
                background="worker",
                skills={
                    SkillType.TECHNICAL: Skill(SkillType.TECHNICAL, level=7),
        ...
        print("   • Designed to test failure scenarios and cascading effects")
    ...
    def display_scenario_status(self):
        ...
        for agent_id, agent in self.game_state.agents.items():
            ...
            stress_level = (
                "🔴" if agent.stress > 70 else "🟡" if agent.stress > 40 else "🟢"
            )
            ...
            print(
                f"     💪 Skills: Combat {getattr(agent.skills.get(SkillType.COMBAT), 'level', 0)}, "
            ...
            if hasattr(agent, "emotional_state"):
                ...
                print(
                    f"     🎯 Combat Effectiveness: {es.get_combat_effectiveness():.2f}"
                )
                print(
                    f"     🗣️ Social Effectiveness: {es.get_social_effectiveness():.2f}"
                )

                if es.trauma_level > 0.5:
                    print(
                        "     ⚠️ HIGH TRAUMA - Mission effectiveness significantly reduced!"
                    )
                if agent.stress > 80:
                    print("     🚨 EXTREME STRESS - Agent at risk of breakdown!")

    def create_test_mission(
        self, mission_type: str, difficulty: str = "hard"
    ) -> Dict[str, Any]:
    ...
    def execute_test_mission(self, mission: Dict[str, Any], selected_agents: List[str]):
        ...
        try:
            ...
            if outcome in [
                result["outcome"].PERFECT_SUCCESS,
                result["outcome"].SUCCESS_WITH_COMPLICATIONS,
            ]:
                print("✅ MISSION SUCCESSFUL!")
                print("   The operation achieved its primary objectives.")
            elif outcome in [
                result["outcome"].PARTIAL_SUCCESS,
                result["outcome"].PARTIAL_SUCCESS_WITH_CONSEQUENCES,
            ]:
                print("🟡 PARTIAL SUCCESS")
                print("   Some objectives achieved, but complications arose.")
            elif outcome in [
                result["outcome"].TRAGIC_SUCCESS,
                result["outcome"].PYRRHIC_VICTORY,
            ]:
            ...
            if consequences:
                ...
                for i, consequence in enumerate(consequences, 1):
                    ...
                    if (
                        hasattr(consequence, "narrative_hooks")
                        and consequence.narrative_hooks
                    ):
            ...
            if "collaboration_analysis" in result:
                ...
                print(
                    f"   Communication Efficiency: {collab.communication_efficiency:.2f}"
                )
                print(f"   Group Cohesion: {collab.group_cohesion:.2f}")

                if collab.group_cohesion < 0.3:
                    print(
                        "   ⚠️ Poor team cohesion contributed to mission difficulties!"
                    )
            ...
            for agent_id, impacts in emotional_impacts.items():
                ...
                for emotion, change in impacts.items():
                    if abs(change) > 0.01:  # Only show significant changes
                        ...
                        print(
                            f"    • {emotion.title()} {change_desc}: {sign}{change:.2f}"
                        )
    ...
    def analyze_cascading_effects(self):
        ...
        high_stress_agents = [
            agent for agent in self.game_state.agents.values() if agent.stress > 70
        ]
        traumatized_agents = [
            agent
            for agent in self.game_state.agents.values()
            if hasattr(agent, "emotional_state")
            and agent.emotional_state.trauma_level > 0.5
        ]

        print("📊 Network Stress Metrics:")
        print(f"   • Average Stress Level: {avg_stress:.1f}/100")
        print(
            f"   • High Stress Agents: {len(high_stress_agents)}/{len(self.game_state.agents)}"
        )
        print(
            f"   • Traumatized Agents: {len(traumatized_agents)}/{len(self.game_state.agents)}"
        )
        ...
        for faction_id, faction in self.game_state.factions.items():
            print(
                f"   • {faction.name}: ${faction.resources.get('money', 0)} "
            ...
            if hasattr(agent, "emotional_state"):
                ...
                if agent.emotional_state.trauma_level > 0:
                    ...
                    if old_trauma != agent.emotional_state.trauma_level:
                        print(
                            f"🩹 {agent.name}: Trauma recovery {old_trauma:.2f} → {agent.emotional_state.trauma_level:.2f}"
                        )
    ...
    def show_detailed_agent_report(self):
        ...
        for agent in self.game_state.agents.values():
            ...
            print("-" * 50)
            print(
                f"📍 Current Location: {self.game_state.locations[agent.location_id].name}"
            )
            ...
            if hasattr(agent, "emotional_state"):
                ...
                print(
                    f"   Fear: {es.fear:.2f} | Anger: {es.anger:.2f} | Trust: {es.trust:.2f}"
                )
                ...
                if es.trauma_level > 0.6:
                    print("   🚨 CRITICAL: Severe trauma affecting all operations")
                elif es.trauma_level > 0.3:
                    print("   ⚠️ WARNING: Moderate trauma reducing effectiveness")

                if not es.is_psychologically_stable():
                    print(
                        "   💔 UNSTABLE: Agent requires immediate psychological intervention"
                    )
    ...
    def apply_random_stress_event(self):
        ...
        for agent in self.game_state.agents.values():
            if agent.status == "active":
                ...
                if hasattr(agent, "emotional_state"):
                    agent.emotional_state.fear = min(
                        1.0, agent.emotional_state.fear + 0.2
                    )
                    agent.emotional_state.trust = max(
                        -1.0, agent.emotional_state.trust - 0.1
                    )
