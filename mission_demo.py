#!/usr/bin/env python3
"""
Years of Lead - Mission Execution System Demo
...
def animate_phase(phase_name):
    ...
    print(f"\n⏳ Executing {phase_name}...", end='', flush=True)
    for _ in range(3):
        time.sleep(0.5)
...
def display_action_log(actions, limit=5):
    """Display recent actions from the mission"""
    print("\n📝 ACTION LOG:")
    print("-" * 60)
    for action in actions[-limit:]:
        symbol = "✅" if action.success else "❌"
        print(f"{symbol} [{action.phase.value.upper()}] {action.narrative}")
    print()


def run_demo():
    """Run the mission execution demo"""
    clear_screen()
    ...
    input("\nPress Enter to receive mission briefing...")
    ...
    input("\n⚠️  Warning: High security location. Press Enter to commence operation...")
    ...
    input("\n\nPress Enter to view mission report...")
    ...
    input("\n\nPress Enter to view detailed action log...")
    
    # Show detailed log
    clear_screen()
    print_header("Detailed Action Log")
    
    for i, action in enumerate(report.action_log):
        symbol = "✅" if action.success else "❌"
        agent_name = next((a.name for a in team if a.id == action.agent_id), "Unknown")
        print(f"{i+1:02d}. {symbol} [{action.phase.value.upper()}] {agent_name}")
        print(f"    {action.narrative}")
        if i < len(report.action_log) - 1:
            print()
...
if __name__ == "__main__":
    try:
        ...
        print("\n\nDemo interrupted. The resistance never sleeps...")
