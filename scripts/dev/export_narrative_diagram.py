...
from game.advanced_relationships import AdvancedRelationshipManager
...
class NarrativeDiagramExporter:
    ...
    def __init__(self, game_state: GameState):
        ...
        self.narrative_engine = game_state.narrative_engine if hasattr(game_state, 'narrative_engine') else None
        self.relationship_manager = game_state.relationship_manager if hasattr(game_state, 'relationship_manager') else None
    ...
    def export_narrative_flow_diagram(self, output_path: str = "narrative_flow.png"):
        ...
        for agent_id, agent in self.game_state.agents.items():
            G.add_node(f"agent_{agent_id}", node_type="agent", color="gold",
                      label=agent.name)
        ...
        for agent_id, agent in self.game_state.agents.items():
            if hasattr(agent, 'memory_journal') and agent.memory_journal:
                for memory in agent.memory_journal:
                    ...
                    G.add_node(memory_node, node_type="memory", color="lightgreen",
        ...
        for agent_id, agent in self.game_state.agents.items():
            if hasattr(agent, 'secrets') and agent.secrets:
                for secret in agent.secrets:
                    ...
                    G.add_node(secret_node, node_type="secret", color="red",
                    ...
                    for knower_id in secret.known_by:
                        if knower_id in self.game_state.agents:
                            G.add_edge(f"agent_{knower_id}", secret_node, relation="knows")
    ...
    def export_symbolic_element_map(self, output_path: str = "symbolic_elements.png"):
        ...
        # Add ideology nodes
        ideology_types = ["radical", "pacifist", "individualist", "traditional", "nationalist", "materialist"]
        for ideology in ideology_types:
            G.add_node(f"ideology_{ideology}", node_type="ideology", color="lightblue")

        # Add emotion nodes
        emotion_types = ["hope", "fear", "anger", "despair", "joy", "trust", "surprise", "sadness"]
        ...
        node_sizes = [800 if G.nodes[node].get('node_type') == 'agent' else 400 for node in G.nodes()]
                ...
                for memory in agent.memory_journal:
                    ...
                    if hasattr(memory, 'symbolic_elements'):
                        for element in memory.symbolic_elements:
                            symbolic_frequency[element] = symbolic_frequency.get(element, 0) + 1

        analytics["narrative_metrics"] = {
            "total_memories": total_memories,
            "memory_emotional_tones": memory_tones,
            "average_memories_per_agent": total_memories / len(self.game_state.agents) if self.game_state.agents else 0
        }

        analytics["symbolic_analysis"] = {
            "symbolic_element_frequency": symbolic_frequency,
            "most_common_symbols": sorted(symbolic_frequency.items(), key=lambda x: x[1], reverse=True)[:5]
        }
...
def visualize_symbol(symbol: str, game_state: Optional[GameState] = None):
    ...
    for agent_id, agent in game_state.agents.items():
        if hasattr(agent, 'memory_journal') and agent.memory_journal:
            for memory in agent.memory_journal:
                if hasattr(memory, 'symbolic_elements') and symbol in memory.symbolic_elements:
                    agent_node = f"agent_{agent.name}"
                    G.add_node(agent_node, node_type="agent", color="gold")
...
if __name__ == "__main__":
    ...
    print(f"Analytics exported with {analytics['narrative_metrics']['total_memories']} total memories")
