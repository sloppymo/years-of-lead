...
async def list_factions(
    ...
    if faction_type and ideology:
        factions = await faction_service.get_factions_by_type_and_ideology(
            faction_type, ideology
        )
    elif faction_type:
...
@router.get(
    "/{faction_id}/relationships", response_model=List[FactionRelationshipResponse]
)
async def get_faction_relationships(
    ...
    if game_id:
        ...
        relationships = await faction_service.get_faction_relationships_in_game(
            game_id, faction_id
        )
    else:
