...
def cli_mission_briefing_menu(
    ...
    while True:
        ...
        if choice == "1":
            ...
            cli_briefing_risk_analysis(
                mission_engine, integration_manager, equipment_manager, agents, missions
            )
        elif choice == "3":
...
def cli_view_missions(missions: list):
    ...
    if not missions:
        ...
        for idx, m in enumerate(missions):
            print(
                f"[{idx+1}] {m['name']} (Type: {m['type']}, Difficulty: {m['difficulty']})"
            )
...
def cli_briefing_risk_analysis(
    mission_engine, integration_manager, equipment_manager, agents, missions
):
    clear_screen()
    ...
    analysis = integration_manager.analyze_mission_equipment(
    ...
    print(
        f"Concealment: {analysis.concealment_rating:.2f}  Legal Risk: {analysis.legal_risk_level:.2f}"
    )
...
def cli_launch_mission(mission_engine, integration_manager, agents, missions):
    ...
    mission = missions[int(choice) - 1]
    confirm = (
        input(f"Are you sure you want to launch '{mission['name']}'? (y/N): ")
        .strip()
        .lower()
    )
    if confirm != "y":
    ...
    result = mission_engine.execute_mission(
        ...
        agent_loadouts={
            a["id"]: integration_manager.get_agent_loadout(a["id"]) for a in agents
        },
    )
    ...
    print(
        f"Equipment Effects: {result['equipment_effects'].get('success_modifier', 0.0):+.1%}"
    )
    input("\nPress Enter to return.")
