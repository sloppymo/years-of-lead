...
class ContextualNarrativeHook:
    ...
    def _evaluate_requirement(self, requirement: str, value: Any, game_state) -> bool:
        ...
        if requirement == "min_traumatized_agents":
            traumatized_count = sum(1 for agent in game_state.agents.values() 
                                  if hasattr(agent, 'emotional_state') and 
                                  agent.emotional_state.trauma_level > 0.6)
            ...
            faction_agents = [a for a in game_state.agents.values() 
...
class EnhancedDynamicNarrativeSystem:
    ...
    def _initialize_contextual_hooks(self):
        ...
        # Trauma-based hooks
        self.contextual_hooks.append(ContextualNarrativeHook(
        ...
        self.contextual_hooks.append(ContextualNarrativeHook(
            ...
            context_requirements={"relationship_conflict": {"agent_a": "any", "agent_b": "any", "min_conflict": -30}},
        ...
        self.contextual_hooks.append(ContextualNarrativeHook(
            ...
            context_requirements={"max_faction_loyalty": {"faction_id": "any", "max_loyalty": 60}},
    ...
    def _check_for_narrative_triggers(self, current_turn: int) -> List[NarrativeArc]:
        ...
        for agent_id, agent in self.game_state.agents.items():
            ...
            if intensity > 0.8 and dominant_emotion in ["fear", "anger", "sadness"]:
                arc = self._create_emotional_breakdown_arc(agent_id, dominant_emotion, current_turn)
                if arc:
                    new_arcs.append(arc)
        ...
        for agent_id, agent in self.game_state.agents.items():
            ...
            for other_id, relationship in agent.relationships.items():
                ...
                if relationship.loyalty < 0.3 and relationship.affinity > 20:
                    arc = self._create_loyalty_crisis_arc(agent_id, other_id, current_turn)
                    if arc:
                        new_arcs.append(arc)
    ...
    def _create_trauma_arc(self, agent_id: str, current_turn: int) -> Optional[NarrativeArc]:
    ...
    def _create_emotional_breakdown_arc(self, agent_id: str, emotion: str, current_turn: int) -> Optional[NarrativeArc]:
    ...
    def _create_betrayal_arc(self, agent_a_id: str, agent_b_id: str, current_turn: int) -> Optional[NarrativeArc]:
    ...
    def _create_loyalty_crisis_arc(self, agent_a_id: str, agent_b_id: str, current_turn: int) -> Optional[NarrativeArc]:
    ...
    def _calculate_arc_advancement_chance(self, arc: NarrativeArc) -> float:
        ...
        return min(0.8, base_chance + complexity_modifier + emotional_modifier + type_modifier)
    
    def _generate_arc_story_element(self, arc: NarrativeArc) -> Dict[str, Any]:
        """Generate a story element for an arc stage"""
        agent_names = [self.game_state.agents.get(aid, type('obj', (), {'name': f'Agent_{aid}'})).name 
        ...
        story_templates = {
            ...
            NarrativeArcType.BETRAYAL: [
                ...
                f"Other agents begin to take sides in the conflict",
                f"The network fractures along lines of loyalty"
            ],
            NarrativeArcType.LOYALTY_TEST: [
                ...
                f"Both agents struggle with their commitments"
            ]
        }
        
        templates = story_templates.get(arc.arc_type, [f"The story continues for {', '.join(agent_names)}"])
    ...
    def _check_for_emergent_plotlines(self, current_turn: int) -> List[EmergentPlotline]:
        ...
        if len(new_plotlines) + len(self.emergent_plotlines) > self.max_plotlines:
            new_plotlines = new_plotlines[:self.max_plotlines - len(self.emergent_plotlines)]
    ...
    def _create_faction_conflict_plotline(self, conflict: Dict[str, Any], current_turn: int) -> Optional[EmergentPlotline]:
        ...
        plotline.resolution_conditions = [
            {"type": "faction_state", "faction_id": conflict["faction_a"], "state": "reconciled"},
            {"type": "faction_state", "faction_id": conflict["faction_b"], "state": "reconciled"}
        ]
    ...
    def _detect_network_patterns(self) -> List[Dict[str, Any]]:
        ...
        for agent_id, agent in self.game_state.agents.items():
            ...
            positive_relationships = sum(1 for rel in agent.relationships.values() 
                                       if rel.affinity > 10)
        ...
        traumatized_agents = []
        for agent_id, agent in self.game_state.agents.items():
            if hasattr(agent, 'emotional_state') and agent.emotional_state.trauma_level > 0.6:
    ...
    def _create_network_pattern_plotline(self, pattern: Dict[str, Any], current_turn: int) -> Optional[EmergentPlotline]:
        ...
        if pattern["type"] == "isolation":
            plotline.resolution_conditions = [
                ...
                for aid in pattern["agents_involved"][:2]  # Require 2 agents to reconnect
            ]
    ...
    def _generate_plotline_development(self, plotline: EmergentPlotline, current_turn: int) -> Dict[str, Any]:
        ...
        templates = development_templates.get(plotline.plotline_type, ["The situation continues to develop"])
        description = random.choice(templates)
    ...
    def _generate_story_events(self, results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for advancement in results["arc_advancements"]:
            story_events.append({
                ...
                "emotional_impact": advancement["story_element"]["emotional_impact"]
            })
    ...
    def _apply_narrative_consequences(self, results: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _apply_story_event_consequence(self, event: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Apply consequence for a single story event"""
        
        if event["type"] == "arc_advancement":
            # Apply emotional impacts to involved agents
            arc = next((a for a in self.narrative_arcs if a.id == event["arc_id"]), None)
            if arc:
                for agent_id in arc.agents_involved:
                    agent = self.game_state.agents.get(agent_id)
                    if agent and hasattr(agent, 'emotional_state'):
    ...
    def get_narrative_summary(self) -> Dict[str, Any]:
        ...
        active_plotlines = [plotline for plotline in self.emergent_plotlines if plotline.active]
    ...
    def _calculate_narrative_efficiency(self) -> float:
        ...
        if participation_scores:
            ...
            participation_variance = sum((score - mean_participation) ** 2 for score in participation_scores) / len(participation_scores)
        
        # Hook utilization
        hook_utilization = len(self.used_hooks) / len(self.contextual_hooks) if self.contextual_hooks else 0.0
