...
class EnhancedEquipmentManager:
    ...
    def register_equipment(self, equipment: EnhancedEquipmentProfile):
        ...
        logger.info(f"Registered enhanced equipment: {equipment.name} ({equipment.item_id})")
    ...
    def get_equipment_by_category(self, category: EquipmentCategory) -> List[EnhancedEquipmentProfile]:
        """Get all equipment in a specific category"""
        return [eq for eq in self.equipment_registry.values() if eq.category == category]

    def get_equipment_by_rarity(self, min_rarity: float = 0.0, max_rarity: float = 1.0) -> List[EnhancedEquipmentProfile]:
        """Get equipment within a rarity range"""
        return [eq for eq in self.equipment_registry.values() if min_rarity <= eq.rarity <= max_rarity]
