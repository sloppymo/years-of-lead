...
from .equipment_enhanced import (
    EnhancedEquipmentProfile, EquipmentDurability, EquipmentEffects,
    EquipmentQuality, EnhancedEquipmentManager
)
...
class AgentLoadout:
    ...
    def add_equipment(self, slot: LoadoutSlot, equipment: EnhancedEquipmentProfile) -> bool:
    ...
    def _recalculate_loadout_stats(self):
        ...
        for equipment in self.equipment.values():
            ...
            for skill, bonus in equipment.effects.skill_bonuses.items():
                self.equipment_bonuses[skill] = self.equipment_bonuses.get(skill, 0.0) + bonus
            
            # Mission modifiers
            for mission_type, modifier in equipment.effects.mission_modifiers.items():
                self.mission_modifiers[mission_type] = self.mission_modifiers.get(mission_type, 0.0) + modifier
            
            # Emotional effects
            for emotion, effect in equipment.effects.emotional_effects.items():
                self.emotional_effects[emotion] = self.emotional_effects.get(emotion, 0.0) + effect
            
            # Social effects
            for context, effect in equipment.effects.social_effects.items():
                self.social_effects[context] = self.social_effects.get(context, 0.0) + effect
    ...
    def use_equipment_in_mission(self, mission_intensity: float = 1.0) -> Dict[str, Any]:
        ...
        for slot, equipment in self.equipment.items():
            ...
            degradation_results[slot.value] = {
                ...
                "degradation": equipment.durability.condition - equipment.durability.condition,
    ...
    def _calculate_slot_intensity(self, slot: LoadoutSlot, mission_intensity: float) -> float:
...
class EquipmentIntegrationManager:
    ...
    def analyze_mission_equipment(
        ...
        success_boost = self._calculate_success_boost(recommended_equipment, mission_type)
    ...
    def _score_equipment_for_mission(
        ...
        for equipment in equipment_list:
            ...
            if mission_type in ["intelligence", "sabotage"]:
                tech_bonus = equipment.get_skill_bonus("hacking") + equipment.get_skill_bonus("intelligence")
    ...
    def _generate_equipment_recommendations(
        ...
        category_to_slot = {
            EquipmentCategory.WEAPON: [LoadoutSlot.PRIMARY_WEAPON, LoadoutSlot.SECONDARY_WEAPON],
            EquipmentCategory.TOOL: [LoadoutSlot.TOOL],
            EquipmentCategory.ELECTRONIC: [LoadoutSlot.ELECTRONIC],
            EquipmentCategory.ARMOR: [LoadoutSlot.DISGUISE],  # Map armor to disguise slot
            ...
            EquipmentCategory.COMMUNICATION: [LoadoutSlot.ELECTRONIC],  # Map communication to electronic
            EquipmentCategory.CURRENCY: [LoadoutSlot.SPECIAL],  # Map currency to special
            EquipmentCategory.CONTRABAND: [LoadoutSlot.SPECIAL]  # Map contraband to special
        }
            ...
            if category in category_to_slot:
                ...
                for i, slot in enumerate(slots):
                    if i < len(sorted_equipment):
                        ...
                        recommendations[slot].extend(sorted_equipment[:3])  # Top 3 options
    ...
    def _create_loadout_suggestions(
        ...
        strategies = [
            {"name": "Stealth", "focus": "concealment", "slots": [LoadoutSlot.TOOL, LoadoutSlot.ELECTRONIC, LoadoutSlot.DISGUISE]},
            {"name": "Combat", "focus": "combat", "slots": [LoadoutSlot.PRIMARY_WEAPON, LoadoutSlot.SECONDARY_WEAPON, LoadoutSlot.MEDICAL]},
            {"name": "Technical", "focus": "intelligence", "slots": [LoadoutSlot.ELECTRONIC, LoadoutSlot.TOOL, LoadoutSlot.DOCUMENT]},
            {"name": "Balanced", "focus": "balanced", "slots": [LoadoutSlot.PRIMARY_WEAPON, LoadoutSlot.ELECTRONIC, LoadoutSlot.TOOL]}
        ]
    ...
    def apply_equipment_effects_to_mission(
        ...
        for agent in agents:
            ...
            if loadout:
                ...
                equipment_effects["success_modifier"] += loadout.get_mission_modifier(mission_type)
                ...
                equipment_effects["concealment_rating"] = max(
                    equipment_effects["concealment_rating"],
                    loadout.concealment_rating
                )
                equipment_effects["legal_risk"] = max(
                    equipment_effects["legal_risk"],
                    loadout.legal_risk
                )
        ...
        for loadout in self.agent_loadouts.values():
            for slot, equipment in loadout.equipment.items():
                ...
                if equipment.durability.condition <= 0.0:
                    ...
                    maintenance_report["repair_costs"] += equipment.durability.repair_cost
