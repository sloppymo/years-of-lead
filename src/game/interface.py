...
from .entities import Agent, Mission, MissionType, GamePhase, Location
from .core import GameState
from .relationships import evaluate_team_dynamics, TeamDynamics, RelationshipEvent
...
class PlayerInterface:
    ...
    def display_header(self):
        ...
        print("=" * 80)
        print(f"Turn: {self.game_state.turn_number} | Phase: {self.game_state.current_phase.value.upper()}")
    ...
    def display_faction_status(self):
        ...
        available_agents = len([a for a in self.game_state.agents.values() if a.status == "available"])
        broken_agents = len([a for a in self.game_state.agents.values() if hasattr(a, 'psychological_state') and a.psychological_state == "broken"])
        ...
        if hasattr(self.game_state, 'faction_cohesion') and self.game_state.faction_cohesion.unity_score < 60:
            print(f"   🚨 FACTION INSTABILITY - Risk of fracture!")
    ...
    def display_main_menu(self) -> str:
        ...
        print(f"   1. Plan New Mission")
        print(f"   2. Review Agent Status") 
        print(f"   3. Review Location Intel")
        print(f"   4. Review Relationship Map")
        print(f"   5. End Planning Phase")
    ...
    def plan_new_mission(self) -> Optional[MissionPacket]:
        ...
        print(f"\n" + "=" * 60)
        print(f"   MISSION PLANNING WIZARD")
        print(f"=" * 60)
        ...
        equipment_assignments = {a.id: [] for a in agents}  # TODO: Implement equipment system
    ...
    def select_agents(self, mission_type: MissionType, location: Location) -> List[Agent]:
        """Select agents for mission with psychological assessment"""
        available_agents = [a for a in self.game_state.agents.values() if a.status == "available"]
        ...
        while len(selected_agents) < max_team_size:
            ...
            self.display_detailed_agent_list(available_agents, mission_type, selected_agents)
            
            print(f"\n   Options:")
            print(f"   • Enter agent number to select/deselect")
            print(f"   • Type 'continue' to proceed with current selection")
            print(f"   • Type 'back' to return to previous step")
    ...
    def display_detailed_agent_list(self, agents: List[Agent], mission_type: MissionType, selected: List[Agent]):
        ...
        print(f"{'#':<2} {'Name':<12} {'Mood':<5} {'Stress':<6} {'Skill':<6} {'Status':<15} {'Warnings'}")
        print(f"{'='*60}")
    ...
    def get_relevant_skill_value(self, agent: Agent, mission_type: MissionType) -> float:
    ...
    def display_team_compatibility_preview(self, agents: List[Agent]):
        ...
        if hasattr(self.game_state, 'evaluate_team_dynamics'):
            ...
            if team_dynamics.synergy_bonus > 0.1:
                print(f"   ✅ Strong team synergy: +{team_dynamics.synergy_bonus:.0%} effectiveness")
            elif team_dynamics.synergy_bonus < -0.1:
                print(f"   ⚠️ Team friction: {abs(team_dynamics.synergy_bonus):.0%} penalty")
            
            if hasattr(team_dynamics, 'conflicts') and team_dynamics.conflicts:
                print(f"   🚨 Relationship issues detected!")
    
    def select_tactical_approach(self, mission_type: MissionType, location: Location, 
                               agents: List[Agent]) -> TacticalApproach:
        ...
        for i, agent_a in enumerate(agents):
            for agent_b in agents[i+1:]:
                ...
                affinity_icon = "💙" if rel.affinity > 60 else "😐" if rel.affinity > 30 else "💔"
                
                print(f"   {agent_a.name} → {agent_b.name}: {trust_icon} Trust {rel.trust:.0f}, {affinity_icon} Affinity {rel.affinity:.0f}")
    ...
    def mission_planning_interface(self, current_missions_count: int = 0) -> Optional[Tuple[Mission, List[Agent]]]:
        """
        ...
        if hasattr(self.game_state, 'planned_missions') and self.game_state.planned_missions:
            print("\n📋 CURRENTLY PLANNED MISSIONS:")
            for i, (m, agents) in enumerate(self.game_state.planned_missions, 1):
                print(f"  {i}. {m.mission_type.value.upper()} with {', '.join(a.name for a in agents)}")
        ...
        try:
            ...
            mission_choice = input("\nSelect mission type (1-{}, or 'c' to cancel): ".format(len(mission_types)))
            ...
            # Get agent selection
            while True:
                agent_selection = input("Select agents (comma-separated numbers, e.g., '1,3,5'): ")
                try:
                    agent_indices = [int(x.strip()) - 1 for x in agent_selection.split(',')]
            ...
            if len(selected_agents) > 1 and hasattr(self, 'display_team_analysis'):
                ...
                if not self.display_team_analysis(selected_agents):
                    print("Mission planning cancelled due to team compatibility issues.")
                    input("Press Enter to continue...")
                    return None
            ...
            print(f"Assigned agents: {', '.join(agent.name for agent in selected_agents)}")
            ...
            return None
    def display_mission_outcome(self, mission: Mission, agents: List[Agent], outcome: Dict):
    ...
    def display_agent_status(self, agent: Agent, detailed: bool = False) -> str:
        ...
        # Basic status line
        status_line = f"[{agent.id}] {agent.name} {emotion_icon}"
        status_line += f" | Stress: {agent.stress}% | Loyalty: {agent.loyalty}%"
        
        if detailed:
            status_line += f"\n    Location: {agent.location_id}"
            status_line += f" | Background: {agent.background}"
            status_line += f"\n    Emotions: "
    ...
    def _calculate_mission_risk(self, mission_type: MissionType, agents: List[Agent]) -> int:
    ...
    def _calculate_mission_risk(self, mission_type: MissionType, agents: List[Agent]) -> int:
        ...
        # Adjust based on agent stress levels
        avg_stress = sum(agent.stress for agent in agents) / len(agents) if agents else 0
        if avg_stress > 70:
            risk += 2
        elif avg_stress > 50:
            risk += 1
        ...
        for i, agent_a in enumerate(agents):
            for agent_b in agents[i+1:]:
                if hasattr(agent_a, 'get_relationship_with'):
                    ...
                    if rel.trust < 30:
                        warnings.append(f"Low trust between {agent_a.name} and {agent_b.name}")
                    if hasattr(rel, 'has_conflict') and rel.has_conflict():
                        warnings.append(f"Active conflict between {agent_a.name} and {agent_b.name}")
    ...
    def display_team_analysis(self, agents: List[Agent]) -> bool:
        ...
        if analysis['synergy'] > 0.1:
            print(f"   ✅ Positive team synergy (+{analysis['synergy']*100:.0f}% effectiveness)")
        elif analysis['synergy'] < -0.1:
            print(f"   ⚠️  Negative team synergy ({analysis['synergy']*100:.0f}% effectiveness)")
        else:
...
class EmotionalStateManager:
    ...
    def update_agent_emotions(self, agent: Agent, mission_outcome: Dict[str, any]) -> Dict[str, float]:
        ...
        if casualties > 0:
            emotional_changes['fear'] = emotional_changes.get('fear', 0) + 0.1 * casualties
            emotional_changes['anger'] = emotional_changes.get('anger', 0) + 0.05 * casualties
