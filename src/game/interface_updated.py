...
class PlayerInterface:
    ...
    def mission_planning_interface(self, current_missions_count: int = 0) -> Optional[Tuple[Mission, List[Agent]]]:
        ...
        try:
            ...
            busy_agents = [agent.name for agent in selected_agents if hasattr(agent, 'current_mission') and agent.current_mission]
            if busy_agents:
    ...
    def display_team_analysis(self, agents: List[Agent]) -> bool:
        ...
        compat_status = "🟢 GOOD" if compat_score >= 70 else "🟡 FAIR" if compat_score >= 40 else "🔴 POOR"
