...
class SaveManager:
    ...
    def __init__(self):
        ...
        self.save_directory = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'saves')
    ...
    def save_game(self, game, filename, save_type="manual"):
        ...
        metadata = {
            ...
            "controlled_locations": sum(1 for loc in game_state.get("locations", []) if loc.get("controlled", False)),
            "save_type": save_type,
    ...
    def display_save_browser(self):
        ...
        for i, filename in enumerate(save_files, 1):
            ...
            if metadata:
                ...
                print(f"{i}. [{save_type.upper()}] Turn {turn} ({date}) - Agents: {agent_count}, Support: {public_support}%, "
