...
class GameStateSnapshot(BaseModel):
    ...
    snapshot_type: str = Field(
        ...,
        description="Type of snapshot (e.g., 'turn_start', 'turn_end', 'manual_save')",
    )

    # Core game state
    current_turn: int = Field(..., description="Current turn number")
    game_status: str = Field(
        ..., description="Game status (running, paused, completed)"
    )

    # Resources and metrics
    resources: Dict[str, int] = Field(
        ..., description="Resource amounts (money, influence, etc.)"
    )
    ...
    player_characters: Dict[str, Dict[str, Any]] = Field(
        ..., description="Player character states"
    )
    operations: Dict[str, Dict[str, Any]] = Field(..., description="Operation states")

    # Relationships
    faction_relationships: Dict[str, Dict[str, int]] = Field(
        ..., description="Faction relationship values"
    )
    district_control: Dict[str, Dict[str, float]] = Field(
        ..., description="Faction control in districts"
    )

    # SYLVA/WREN integration data
    emotional_states: Dict[str, Any] = Field(
        {}, description="SYLVA emotional state data"
    )
    narrative_context: Dict[str, Any] = Field(
        {}, description="WREN narrative context data"
    )
    symbolic_markers: List[Dict[str, Any]] = Field(
        [], description="Symbolic elements for storytelling"
    )

    class Config:
        schema_extra = {"collection": "game_state_snapshots"}
...
class GameEventLog(BaseModel):
    ...
    affected_entities: Dict[str, List[str]] = Field(
        {}, description="IDs of entities affected by event"
    )
    initiator: Optional[Dict[str, str]] = Field(
        None, description="Entity that initiated the event"
    )
    ...
    emotional_impact: Dict[str, Any] = Field(
        {}, description="SYLVA emotional impact data"
    )
    symbolic_elements: List[Dict[str, Any]] = Field(
        [], description="Symbolic elements from event"
    )

    class Config:
        schema_extra = {"collection": "game_event_logs"}
...
class PlayerJournal(BaseModel):
    ...
    emotional_tags: List[Dict[str, Any]] = Field(
        [], description="Emotional tags from SYLVA"
    )
    emotional_state: Dict[str, Any] = Field({}, description="Emotional state snapshot")
    narrative_context: Dict[str, Any] = Field(
        {}, description="Narrative context from WREN"
    )
    symbolic_elements: List[Dict[str, Any]] = Field(
        [], description="Symbolic narrative elements"
    )
    character_growth_markers: List[Dict[str, Any]] = Field(
        [], description="Character development indicators"
    )

    class Config:
        schema_extra = {"collection": "player_journals"}
...
class FactionAnalytics(BaseModel):
    ...
    influence_by_district: Dict[str, float] = Field(
        {}, description="Influence per district"
    )
    ...
    average_relationship: float = Field(
        0.0, description="Average relationship with other factions"
    )
    relationships_delta: Dict[str, int] = Field(
        {}, description="Relationship changes this turn"
    )

    # AI behavior data
    decision_factors: Dict[str, float] = Field(
        {}, description="Factors affecting AI decisions"
    )
    strategy_focus: Dict[str, float] = Field({}, description="Strategic focus areas")

    class Config:
        schema_extra = {"collection": "faction_analytics"}
...
class WorldState(BaseModel):
    ...
    media_focus: List[Dict[str, Any]] = Field(
        [], description="Current media focus areas"
    )
    ...
    policy_changes: List[Dict[str, Any]] = Field(
        [], description="Recent policy changes"
    )

    # Environmental factors
    season: str = Field("spring", description="Current season")
    weather_conditions: Dict[str, Any] = Field(
        {}, description="Weather conditions by district"
    )

    class Config:
        schema_extra = {"collection": "world_states"}
...
class AITrainingData(BaseModel):
    ...
    quality_score: Optional[float] = Field(
        None, description="Quality score if evaluated"
    )

    class Config:
        schema_extra = {"collection": "ai_training_data"}
...
class SylvaIntegrationData(BaseModel):
    ...
    emotional_state: Dict[str, float] = Field(
        ..., description="Current emotional state values"
    )
    emotional_trajectory: List[Dict[str, Any]] = Field(
        [], description="Recent emotional changes"
    )
    psychological_profile: Dict[str, Any] = Field(
        {}, description="Character psychological profile"
    )

    # Symbolic elements
    symbolic_patterns: List[Dict[str, Any]] = Field(
        [], description="Detected symbolic patterns"
    )
    narrative_dissonance: Dict[str, Any] = Field(
        {}, description="Narrative dissonance metrics"
    )
    dream_elements: List[Dict[str, Any]] = Field(
        [], description="Elements for dream sequences"
    )

    # Therapeutic data
    therapy_suggestions: List[Dict[str, Any]] = Field(
        [], description="Suggested therapeutic narratives"
    )
    emotional_regulation_metrics: Dict[str, float] = Field(
        {}, description="Emotional regulation metrics"
    )

    class Config:
        schema_extra = {"collection": "sylva_integration_data"}
...
class WrenIntegrationData(BaseModel):
    ...
    character_id: Optional[str] = Field(
        None, description="UUID of character if applicable"
    )
    timestamp: datetime = Field(default_factory=datetime.now, description="Timestamp")

    # Narrative elements
    active_narrative_arcs: List[Dict[str, Any]] = Field(
        [], description="Active narrative arcs"
    )
    character_arcs: Dict[str, Dict[str, Any]] = Field(
        {}, description="Character development arcs"
    )
    world_narrative_threads: List[Dict[str, Any]] = Field(
        [], description="World narrative threads"
    )

    # Reflective elements
    reflective_prompts: List[Dict[str, Any]] = Field(
        [], description="Reflective roleplay prompts"
    )
    narrative_reflections: List[Dict[str, Any]] = Field(
        [], description="Narrative reflections"
    )
    character_growth_opportunities: List[Dict[str, Any]] = Field(
        [], description="Character growth opportunities"
    )

    # Therapeutic narratives
    therapeutic_narratives: List[Dict[str, Any]] = Field(
        [], description="Generated therapeutic narratives"
    )
    trauma_informed_contexts: Dict[str, Any] = Field(
        {}, description="Trauma-informed narrative contexts"
    )

    class Config:
        schema_extra = {"collection": "wren_integration_data"}
