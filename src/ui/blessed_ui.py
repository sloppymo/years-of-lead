...
class BlessedUI:
    ...
    def draw_status_bar(self):
        ...
        money = (
            next(iter(gs.factions.values())).resources.get("money", 0)
            if gs.factions
            else 0
        )
        print(
            ...
            + term.bold_white_on_blue(
                f" {district}     Day: {day}     Phase: {phase}     Money: ${money} ".center(
                    term.width, "═"
                )
            )
        )
    ...
    def draw_cell_panel(self):
        ...
        print(
            term.move_yx(y, 0)
            + term.bold("╔════════ ACTIVE CELL: Forward House ════════╗")
        )
        y += 1
        ...
        if self.agent_keys:
            ...
            loc = (
                gs.locations[agent.location_id].name[:10]
                if agent.location_id in gs.locations
                else agent.location_id
            )
            task = agent.task_queue[0].task_type.value if agent.task_queue else "[None]"
            highlight = (
                term.reverse if idx == self.selected_agent_idx else (lambda x: x)
            )
            print(
                term.move_yx(y + 1, 0)
                + highlight(
                    f"{agent.name[:12]:12} {agent.skill_level:2}/10  {loc:10} {agent.status:8} {task:12}"
                )
            )
    ...
    def draw_location_panel(self):
        ...
        print(
            term.move_yx(y, x)
            + highlight(term.bold(f"╔══ LOCATION: {loc.name} [{loc.id.upper()}] ══╗"))
        )
        y += 1
        print(
            term.move_yx(y, x)
            + f"Security: {bar(loc.security_level, 10)} ({loc.security_level}/10)"
        )
        y += 1
        print(
            term.move_yx(y, x)
            + f"Unrest:   {bar(loc.unrest_level, 10)} ({loc.unrest_level}/10)"
        )
        y += 1
        print(term.move_yx(y, x) + "Control:  [NEUTRAL]")
        y += 1
        print(term.move_yx(y, x) + "Presence: Police••• Students•••• Workers•")
        y += 1
    ...
    def draw_mission_panel(self):
        ...
        print(
            term.move_yx(y, 0)
            + term.bold("╔════ CURRENT MISSION: Infiltrate Government Office ════╗")
        )
        y += 1
        print(
            term.move_yx(y, 0)
            + "Risk Level: [███░░] (3/5)  |  Progress: [██░░░░░░░░] (2/10)"
        )
        y += 1
            ...
            while self.running:
                ...
                if key:
                    if key.lower() == "q":
                        ...
                        self.selected_agent_idx = (self.selected_agent_idx - 1) % max(
                            1, len(self.agent_keys)
                        )
                        ...
                        self.selected_location_idx = (
                            self.selected_location_idx - 1
                        ) % max(1, len(self.location_keys))
                    elif key.name == "KEY_RIGHT":
                        self.selected_location_idx = (
                            self.selected_location_idx + 1
                        ) % max(1, len(self.location_keys))
                    elif key.name == "KEY_PGUP":
                        self.event_log_offset = min(
                            self.event_log_offset + 1,
                            max(0, len(self.game_state.narrative_log) - 5),
                        )
