...
from src.game.entities import Agent, Mission, MissionType, Location
from src.game.relationships import Relationship
...
class TestEnhancedMissionFlows(unittest.TestCase):
    ...
    def test_mission_execution_flow(self):
        ...
        with patch('src.game.mission_flow_integration.execute_enhanced_mission') as mock_execute:
            ...
            self.assertEqual(outcome['description'], 'Test mission completed successfully')
            
            # Verify agent states were updated
            # Note: current_missions is cleared after mission execution
            self.assertNotIn(self.mission.id, getattr(self.agent1, 'current_missions', []))
            self.assertNotIn(self.mission.id, getattr(self.agent2, 'current_missions', []))
    ...
    def test_fallback_to_basic_mission(self):
        ...
        with patch('src.game.mission_flow_integration.execute_enhanced_mission', 
