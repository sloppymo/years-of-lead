...
def test_cli_victory_defeat():
    ...
    output = run_cli_with_inputs(
        ["v", "d", "q"]
    )  # trigger victory, then defeat, then quit
...
def test_cli_error_recovery():
    ...
    assert (
        "Invalid command" in output
        or "not recognized" in output
...
def test_cli_save_metadata():
    ...
    output = run_cli_with_inputs(
        ["s", "1", "", "q"]
    )  # save with empty name (should autogenerate)
...
def test_cli_complex_save_load_flow():
    ...
    output = run_cli_with_inputs(
        ["s", "1", "save1", "s", "1", "save2", "s", "2", "1", "s", "3", "2", "q"]
    )
...
def test_cli_memory_management():
    ...
    output = run_cli_with_inputs(
        ["s", "1", "test1", "s", "1", "test2", "s", "3", "1", "s", "1", "test3", "q"]
    )
