...
def run_cli_with_inputs(inputs):
    ...
    cli = MockCLI()
    with mock.patch.object(builtins, "input", side_effect=inputs), mock.patch(
...
def test_emotional_state_menu():
    ...
    assert (
        "Emotional State" in output or "trauma" in output.lower() or "Events" in output
    )
...
def test_economy_menu():
    ...
    assert (
        "Economy" in output
        or "acquisition" in output.lower()
        or "Mission Briefing" in output
    )
