        ...
        with patch("builtins.input", side_effect=inputs):
            with patch.object(self.cli, "display_header"):
                with patch.object(self.cli, "display_current_status"):
                    with patch.object(self.cli, "display_menu"):
                        with patch.object(
                            self.cli, "display_progress_tracking"
                        ) as mock_progress:
                            # Run the CLI
                            self.cli.run()
        ...
        with patch("builtins.input", side_effect=inputs):
            with patch.object(self.cli, "display_header"):
                with patch.object(self.cli, "display_current_status"):
                    with patch.object(self.cli, "display_menu"):
                        with patch.object(
                            self.cli, "check_victory_conditions", return_value=True
                        ):
                            with patch.object(
                                self.cli, "display_victory_screen"
                            ) as mock_victory:
                                # Run the CLI
                                self.cli.run()
        ...
        with patch("builtins.input", side_effect=inputs):
            with patch.object(self.cli, "display_header"):
                with patch.object(self.cli, "display_current_status"):
                    with patch.object(self.cli, "display_menu"):
                        with patch.object(
                            self.cli, "check_victory_conditions", return_value=False
                        ):
                            with patch.object(
                                self.cli, "check_defeat_conditions", return_value=True
                            ):
                                with patch.object(
                                    self.cli, "display_defeat_screen"
                                ) as mock_defeat:
                                    # Run the CLI
                                    self.cli.run()
                ...
                with patch.object(self.cli, "display_current_status"):
                    with patch.object(self.cli, "display_menu"):
                        with patch.object(
                            ...
                            with patch.object(
                                self.cli, "check_defeat_conditions", return_value=False
                            ):
                                with patch.object(self.cli, "advance_turn"):
                                    ...
                                    self.cli.save_manager.autosave.assert_called_once_with(
                                        self.game
                                    )
