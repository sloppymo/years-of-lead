...
class ArchetypeEngine:
    ...
    def __init__(self, user_profile_path: Optional[str] = None):
            ...
            "MARROW": {
                "the_ember": {
                    ...
                    "responses": [
                        ...
                        "What burns within cannot be extinguished.",
                    ],
                },
                "the_spiral": {
                    "keywords": [
                ...
                "the_seed": {
                    ...
                    "responses": [
                        "The seed knows its season.",
                        "What is planted in darkness grows toward light.",
                    ],
                },
                "the_well": {
                    "keywords": [
                    ...
                    "responses": [
                        "The well runs deeper than you know.",
                        "Ancient waters flow in deep places.",
                    ],
                },
            },
            "ROOT": {
                ...
                "the_river": {
                    ...
                    "responses": [
                        ...
                        "Water shapes stone through patience.",
                    ],
                },
                "the_bridge": {
                    "keywords": [
                    ...
                    "responses": [
                        "The bridge connects what was separate.",
                        "Crossing requires courage.",
                    ],
                },
            },
            "AURA": {
                "the_mask": {
                    ...
                    "responses": [
                        "The mask protects what is tender.",
                        "What is hidden has its reasons.",
                    ],
                },
                "the_tide": {
                    "keywords": [
                ...
                "the_moon": {
                    ...
                    "responses": [
                        "The moon shows all its faces in time.",
                        "Night wisdom differs from day wisdom.",
                    ],
                },
                "the_mirror": {
                    "keywords": [
                    ...
                    "responses": [
                        ...
                        "Reflection reveals what was always there.",
                    ],
                },
            },
        }
        ...
        if user_profile_path:
            ...
            self.profile_path = (
                Path(__file__).parent.parent / "memory" / "user_archetype_profile.json"
            )
    ...
    def _load_user_profile(self) -> Dict:
        ...
        return {
            ...
            "archetype_preferences": {
                subsystem: {} for subsystem in self.archetypes.keys()
            },
    ...
    def track_archetype_preference(
        self, user_input: str, subsystem: str, archetype_used: str
    ):
        """
        ...
        for archetype_name, archetype_data in self.archetypes.get(
            subsystem, {}
        ).items():
            keyword_matches = sum(
                1 for keyword in archetype_data["keywords"] if keyword in user_lower
            )
            if keyword_matches > 0:
                archetype_prefs[archetype_name] = (
                    archetype_prefs.get(archetype_name, 0) + keyword_matches
                )
    ...
    def detect_session_type(self, session_inputs: List[str]) -> str:
        ...
        for session_type, data in self.session_types.items():
            score = sum(
                1 for indicator in data["indicators"] if indicator in combined_text
            )
    ...
    def detect_symbolic_arc(
        self, memory_entries: List[Dict], session_threshold: int = 5
    ) -> Optional[Dict]:
        """
        ...
        emotions = [
            entry.get("emotion") for entry in recent_entries if entry.get("emotion")
        ]
        subsystems = [
            entry.get("subsystem") for entry in recent_entries if entry.get("subsystem")
        ]
        ...
        # Middle pattern: sustained engagement with one subsystem
            ...
            len(subsystems) >= 3
            and subsystems.count(max(set(subsystems), key=subsystems.count)) >= 3
        ):
            dominant_subsystem = max(set(subsystems), key=subsystems.count)
            arc_type = "middle"
            arc_description = (
                f"Deep work in {dominant_subsystem} - the middle passage unfolds"
            )
        ...
        if arc_type:
            ...
            self.user_profile["symbolic_arcs"] = self.user_profile["symbolic_arcs"][
                -10:
            ]
            self._save_user_profile()
    ...
    def calculate_memory_weight(
        self, user_input: str, emotion: Optional[str], subsystem: str
    ) -> float:
        """
        ...
        intensity_count = sum(
            1 for keyword in intensity_keywords if keyword in text_lower
        )
    ...
    def get_user_summary(self) -> Dict:
        ...
        dominant_session_type = (
            max(session_patterns.items(), key=lambda x: x[1])[0]
            if any(session_patterns.values())
            else "balanced"
        )

        # Recent arc analysis
        recent_arcs = (
            self.user_profile["symbolic_arcs"][-3:]
            if self.user_profile["symbolic_arcs"]
            else []
        )
